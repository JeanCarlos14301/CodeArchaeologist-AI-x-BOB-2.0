...
description: >
  Pre-PR risk simulator. Given a proposed code change (diff or description), 
  maps hidden structural dependencies and predicts cascade failures BEFORE 
  any human reviews the PR. Shift-Left risk gate. Never generates code —
  only predicts blast radius, failure modes, and required regression tests.
