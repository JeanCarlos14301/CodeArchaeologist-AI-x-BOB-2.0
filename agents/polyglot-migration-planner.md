...
description: >
  Plans cross-language and cross-framework migrations (e.g. FastAPI → NestJS, 
  Flask → Express, Django → Spring Boot, monolith → microservices). Analyzes 
  source stack, maps concepts to target idioms, and produces a phased migration 
  roadmap with characterization contracts. Use for any non-trivial framework jump.
