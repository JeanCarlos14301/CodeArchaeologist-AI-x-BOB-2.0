...
def build_assess_prompt(stack: StackReport, request: AssessRequest) -> str:
    ...
    return ASSESS_PROMPT.format(
        rules=_RULES, mode_rules=mode_rules, stack=_stack_digest(stack),
        request=json.dumps(request.model_dump(), ensure_ascii=False, indent=1),
    )


def build_plan_prompt(stack: StackReport, request: AssessRequest, assessment: Assessment) -> str:
    ...
    return PLAN_PROMPT.format(
        rules=_RULES, stack=_stack_digest(stack),
...
def validate_assessment(raw: dict[str, Any], request: AssessRequest, stack: StackReport, workspace: Path) -> Assessment:
    ...
    if request.mode == "chosen" and assessment.recommended:
        raise PlannerError("La persona ya eligió destinos: `recommended` debe ir vacío.")
...
def _ask(runner: Runner, prompt: str) -> tuple[dict[str, Any], BobResult]:
    try:
        result = runner.run(PLANNER_MODE, prompt)
    except BobError as exc:
        logger.warning("Bob falló en %s: %s", PLANNER_MODE, exc)
        raise PlannerError("Bob no pudo completar la evaluación. Inténtalo de nuevo en unos minutos.") from exc
    return extract_json(result.last_message), result


def run_assessment(runner: Runner, stack: StackReport, request: AssessRequest, workspace: Path) -> Assessment:
    raw, result = _ask(runner, build_assess_prompt(stack, request))
    assessment = validate_assessment(raw, request, stack, workspace)
...
def run_plan(runner: Runner, stack: StackReport, request: AssessRequest, assessment: Assessment, workspace: Path) -> Plan:
    raw, result = _ask(runner, build_plan_prompt(stack, request, assessment))
    plan = validate_plan(raw, workspace)
