...
interface Props {
  ...
  jobs: Job[];
  error: string | null;
  onStartSample: (sample: string, mode: ExecutionMode) => void;
...
export function HomeView(props: Props) {
  ...
  const launch = () => {
    if (source === "demo") {
      ...
      else onStartSample(demoSample.id, mode);
    } else if (source === "holdout") {
      if (holdoutSample && !offline) onStartSample(holdoutSample.id, mode);
  ...
  const canLaunch = !busy && (source !== "zip" || file !== null);
